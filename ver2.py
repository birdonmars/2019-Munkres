...
def improved_munkres(s, e, Max, N, k):
    ...
    centroid[0] = centroid[0]/k
    centroid[1] = centroid[1]/k
    ...
    absolute_dist = {}
    for i in range(N):
        absolute_dist[i] = dist(s[i], centroid)

    absolute_dist = sorted(absolute_dist.items(), key=operator.itemgetter(1), reverse=True)
    # print(absolute_dist)
    ...
    for (a, b) in absolute_dist:
        temp_idx = costMat[a].index(min(costMat[a][1:]))-1  # 최솟값의 인덱스 반환
        # 겉에서 부터 긁는다
        while count[temp_idx] >= idxcnt[temp_idx]:
            costMat[a][temp_idx + 1] = INF
            temp_idx = costMat[a].index(min(costMat[a][1:])) - 1

        count[temp_idx] += 1
        start_idx[a] = temp_idx
